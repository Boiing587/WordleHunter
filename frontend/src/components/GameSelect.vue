...
let game_selection = ref<GameNames>({
  gen1: [],
  gen2: [],
  gen3: [],
  gen4: [],
...
<template>
  <div class="flex flex-col justify-center items-center">
    <div v-if="game_list" class="flex flex-row justify-center flex-wrap pb-6">
      <div
        v-for="(games, gen) in game_list"
        :key="gen"
        class="min-w-80 game-select-dropdown p-4 text-center"
      >
        <h1 class="pb-4">
          {{ gen.toString().replace('gen', 'Generation ').replace('frontier', 'Frontier') }}
        </h1>
        <MultiSelect
          :id="gen"
          v-model="game_selection[gen]"
          :options="games"
          optionLabel="name"
    ...
    <Button
      :label="anyGamesSelected() ? 'Select all' : 'Clear all'"
      :onClick="anyGamesSelected() ? selectAll : clearGameSelection"
    />
    <Divider class="py-6" />

    <ConfirmDialog style="width: 50%"></ConfirmDialog>
    <ToggleButton
      :model-value="casual_mode_enabled"
      off-label="Normal mode"
      on-label="Casual mode"
      @change="casualModeToggle"
    />
    <ButtonGroup class="p-4">
      <Button label="Daily" @click="submitGameSelection('daily')" :disabled="anyGamesSelected()" />
      <Button
        label="Unlimited"
        @click="submitGameSelection('unlimited')"
        :disabled="anyGamesSelected()"
      />
    </ButtonGroup>
  </div>
</template>
